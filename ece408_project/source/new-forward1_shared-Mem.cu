...
// optimization 1: tiled shared memory
...
#define k4d(i3, i2, i1, i0) k[(i3) * (C * K * K) + (i2) * (K * K) + (i1) * (K) + i0]
    ...
    extern __shared__ float shared_memory[];  // dynamically allocated shared memory
    float* X_shared = &shared_memory[0];
    float* W_shared = &shared_memory[X_tile_width*X_tile_width];

    int n = bx;  // batch
    int m = by;  // output features
    int h0 = tx; // from chapter 16, page 15
    int w0 = ty;
    ...
    float acc = 0.0;
    for (int c=0; c < C; c++) {
      // 1. load Kernel k into shared_memory
      if ((h0 < K) && (w0 < K)) {
        W_shared[h0 * K + w0] = k4d(m, c, h0, w0);
      }
      __syncthreads();

      // 2. load tile from input x into shared_memory
      ...
      // 3. compute partial result of output y
      for (int p = 0; p < K; p++) {
        for (int q = 0; q < K; q++) {
...
__host__ void GPUInterface::conv_forward_gpu(float *device_y, const float *device_x, const float *device_k, const int B, const int M, const int C, const int H, const int W, const int K)
{
    ...
    dim3 gridDim(B, M, Z);
    int X_tile_width = TILE_WIDTH + K -1;
    size_t share_memory_size = sizeof(float) * (X_tile_width * X_tile_width + K * K);
    conv_forward_kernel<<<gridDim, blockDim, share_memory_size>>>(device_y, device_x, device_k, B, M, C, H, W, K);
